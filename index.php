...
<link rel="author" href="https://plus.google.com/+jensmøllerjensz12" />
  ...
  <style type="text/css">
    ...
    .corners .tl {
      border-left-color: {color:Main Theme};
    }
    .corners .tr {
      border-right-color: {color:Main Theme};
    }
    .corners .bl,
    .corners .br {
      border-bottom-color:{color:Main Theme};
    }
    #group-members li span:after,
    #followed li span:after {
      border-top-color:{color:Main Theme};
    }

    /* Outside Link Color */
    a, #footer-end a:hover {
      color:{color:Outside Link};
    }

    /* Outside Link Hover */
    a:hover {
      color:{color:Outside Link Hover};
    }
    ...
    ol.notes a,
    #disqus a {
      color:{color:Inside Link};
    }

    .copy a:hover,
    ol.notes a:hover {
      border-color:{color:Inside Link};
    }
    ...
    #dsq-options a:hover {
      background-color:{color:Inside Link Hover};
    }
    ...
    .photo-btns a:hover,
    #back-to-top .icon {
      background-color:{color:Inside Link};
    }
    ...
    #group-members li span,
    #followed li span {
      text-shadow:1px 1px {color:Colored Area Text Shadow};
    }

    ol.notes li.tumblelog_{PlaintextName} {
      background-color:#fbfbfb;
    }
    ol.notes li.tumblelog_{PlaintextName} .action {
      font-weight:bold;
    }
    #page, .inner {
      margin:{block:IfCenteredLayout}0 auto{/block:IfCenteredLayout}{block:IfNotCenteredLayout}0 0 0 40px{/block:ifNotCenteredLayout};
    }
...
<body class="{block:IfContentWidth500}content-500 {/block:IfContentWidth500}{block:IfContentWidth600}content-600 {/block:IfContentWidth600}{block:IfContentWidth700}content-700 {/block:IfContentWidth700}{block:IndexPage}{block:IfInfiniteScrolling}infscroll {/block:IfInfiniteScrolling}{/block:IndexPage}{block:IfInvert}invert {/block:IfInvert}{block:IfInvertTopBar}invert-topbar {/block:IfInvertTopBar}{block:IfTopMenuWithLabels}menu-labels {/block:IfTopMenuWithLabels}{block:IfAlignTopMenuLeft}menu-left {/block:IfAlignTopMenuLeft}{block:IfRoundedPostCorners}round-corners {/block:IfRoundedPostCorners}{block:IfLargeSocialIcons}lg-icons {/block:IfLargeSocialIcons}{block:IfAutomaticHeadings}auto-headings {/block:IfAutomaticHeadings}{block:IfScaleImages}scale-images {/block:IfScaleImages}{block:PermalinkPage}singular {/block:PermalinkPage}{block:IfHideSidebar}hide-sidebar{/block:IfHideSidebar}">
  <div id="header" class="pinned">
    <div class="inner">
      ...
      {/block:IfNotSearchBoxInSidebar}
      <ul class="menu">
        {block:IfShowRandomPostIcon}<li><a class="random" href="/random"{block:IfNotTopMenuWithLabels} title="{lang:Random post}"{/block:IfNotTopMenuWithLabels}><span class="icon"></span><span class="label">Tilfældig</span></a></li>{/block:IfShowRandomPostIcon}
        <li><a class="archives" href="/archive"{block:IfNotTopMenuWithLabels} title="Arkiv"{/block:IfNotTopMenuWithLabels}><span class="icon"></span><span class="label">Arkiv</span></a></li>
    ...
    <div id="content">
      ...
      {block:Posts}
        <article id="post-{postID}" class="post
          {block:Text}type-text {/block:Text}
          {block:Photo}type-photo {/block:Photo}
          {block:Photoset}type-photoset {/block:Photoset}
          {block:Quote}type-quote {/block:Quote}
          {block:Link}type-link {/block:Link}
          {block:Chat}type-chat {/block:Chat}
          {block:Video}type-video {/block:Video}
          {block:Audio}type-audio {/block:Audio}
          {block:Answer}type-answer {/block:Answer}
          {block:Tags}tag_{URLSafeTag} {/block:Tags}">
          ...
          {block:ContentSource}
            <div class="copy source">
              <p>Kilde: <a href="{SourceURL}">{SourceTitle}</a></p>
            </div>
          {/block:ContentSource}
          ...
          {block:Date}
            <div class="meta">
              <ul class="meta-list">
                ...
                {block:IfShowGroupBlogInfo}<li class="post-author"><img alt="{PostAuthorName} Avatar" src="{PostAuthorPortraitURL-16}">Skrevet af <a href="{PostAuthorURL}">{PostAuthorName}</a></li>{/block:IfShowGroupBlogInfo}

                <li class="date{block:RebloggedFrom}-reblogged{/block:RebloggedFrom}">
                  <a href="{Permalink}" title="{ShortDayOfWeek}. {DayOfMonth}, {Month}, {Year} klokken {24Hour}:{Minutes}">
                    <span class="icon"></span>{TimeAgo}
                  </a>
                  {block:RebloggedFrom} <em>&gt;</em> <a class="reblogged-from" href="{ReblogRootURL}" title="{ReblogRootTitle}">{ReblogRootName}</a>{/block:RebloggedFrom}
                </li>
                ...
                {block:IfDisqusShortname}
                  <li class="comments hidden">
                    <a id="dsq-{PostID}" href="{Permalink}#disqus_thread">
                      <span class="icon"></span>Kommentarer
                    </a>
                  </li>
                {/block:IfDisqusShortname}

                <li class="permalink"><a href="{Permalink}"><span class="icon"></span>Permanent link</a></li>

              </ul><!--/.meta-list -->

              <div class="share">
                <span class="toggle">Del</span>
                <div class="widget">
            ...
            {block:NoteCount}
              <div id="notes">
                <h2>{NoteCount} Notes<span class="sep">/</span>
                  <a id="notes-toggle" href="#"><span class="label">Skjul</span><span class="icon up"></span></a></h2>
                {PostNotes}
              </div><!-- /.notes -->
            {/block:NoteCount}
      ...
      {block:NoSearchResults}
        <div class="post type-text not-found">
          <div class="post-panel">
            <div class="copy">
              <h2 class="post-title">Din søgning gav ingen resultater</h2>
      ...
      {block:Pagination}
        <div id="pagination">
          ...
          <div class="nextprev">
            {block:PreviousPage}<a class="prev" href="{PreviousPage}">{/block:PreviousPage}<span>&larr; Nyere</span>{block:PreviousPage}</a>{/block:PreviousPage} <span class="sep">&bull;</span>
            {block:NextPage}<a class="next" href="{NextPage}">{/block:NextPage}<span>Ældre &rarr;</span>{block:NextPage}</a>{/block:NextPage}
          </div>
        </div><!--/#pagination -->
      {/block:Pagination}
      ...
      {block:PermalinkPagination}
        <div id="pagination">
          <div class="nextprev">
            {block:NextPost}<a class="prev" href="{NextPost}">{/block:NextPost}<span>&larr; Næste</span>{block:NextPost}</a>{/block:NextPost} <span class="sep">&bull;</span>
            {block:PreviousPost}<a class="next" href="{PreviousPost}">{/block:PreviousPost}<span>Forrige &rarr;</span>{block:PreviousPost}</a>{/block:PreviousPost}
          </div>
        </div><!--/#pagination -->
      {/block:PermalinkPagination}
        ...
        <div id="blog-info" class="side-box">
          ...
          {block:IfShowDescription}
            <div class="description">
              {block:IfShowAboutHeading}<h2>Om</h2>{/block:IfShowAboutHeading}
        ...
        {block:IfShowSocialIconsWidget}
          <div class="side-box ruled-top">
            ...
            <ul id="socialize">
              ...
              {block:IfSkypeUsername}<li class="skype"><a href="skype:{text:Skype Username}?userinfo" title="My Skype Info">Skype</a></li>{/block:IfSkypeUsername}
              ...
              {block:IfGithubUsername}<li class="github"><a href="http://github.com/{text:github Username}" title="{text:github Username} on github">{text:github Username} på github</a></li>{/block:IfGithubUsername}
              {block:IfXboxLiveProfileURL}<li class="xbox"><a href="{text:Xbox Live Profile URL}" title="Xbox Live Profile">Xbox profil</a></li>{/block:IfXboxLiveProfileURL}
            </ul>
          </div>
        {/block:IfShowSocialIconsWidget}
          ...
          {block:IfShowTweets}
            {block:Twitter}
              <div id="twitter" class="side-box tab">
                <p class="loading-text">Indlæser tweets&hellip;</p>
              </div>
            {/block:Twitter}
          {/block:IfShowTweets}
          ...
          {block:IfShowInstagram}
            {block:IfInstagramAccessToken}
              <div id="instagram" class="side-box tab">
                <p class="loading-text">Indlæser billeder&hellip;</p>
              </div>
            {/block:IfInstagramAccessToken}
          {/block:IfShowInstagram}
        </div><!-- close TABS -->
          ...
          {block:IfShowLikesInSidebar}
            {block:Likes}
              <div id="side-likes" class="side-box ruled-top">
                ...
                <a class="more-likes" href="http://tumblr.com/liked/by/{Name}">{Flere} Flere &rarr;</a>
              </div>
            {/block:Likes}
          {/block:IfShowLikesInSidebar}
        {/block:IfShowLikes}
        ...
        {block:IfNotShowLikesInSidebar}
          ...
          {block:Likes}
            <div id="likes-panel" class="footer-panel ruled-top">
              <h2>{text:Liked Posts Heading}</h2>
              <a class="more-likes" href="/liked/by/{Name}"><a>Flere &rarr;</a>
              {block:IfContentWidth500}
                {Likes limit="4" summarize="150" width="170"}
              {/block:IfContentWidth500}
      ...
      <div id="footer-end" class="ruled-top">
        <div class="footer-copy">
          <ul id="footer-links">
            ...
            <li><a href="/random">Tilfældig</a></li>
...
<!--  
 ...
 __ \ \  \ \  \_|/_\ \  \\ \  \ \_____  \   /  / ||/ \ \  \__|//  / /   
|\  \\_\  \ \  \_|\ \ \  \\ \  \|____|\  \ /  /_/__   \ \  \  /  /_/__  
\ \________\ \_______\ \__\\ \__\____\_\  \\________\  \ \__\|\________\
 \|________|\|_______|\|__| \|__|\_________\|_______|   \|__| \|_______|
                                \|_________|                            
-->
